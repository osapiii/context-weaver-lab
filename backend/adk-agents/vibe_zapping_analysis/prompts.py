...
- クローラー前提で推測しないでください。ザッピング動画とFileSpaceナレッジを第一情報源にしてください。
- 簡易実装では動画ファイル本体を分割・取得しません。`analysis_evidence` に集約された `transcriptText` / `transcriptSummary` / `quickScan` / `frameCaptures` を第一情報源として解析してください。
- 動画ファイル本体が大きい、または取得できない場合でも、それだけを理由に storyCandidates を空にしてはいけません。文字起こし・簡易スキャン・5秒ごとのframeCapturesから確認できる操作を根拠にUser Storyを生成してください。
- evidence の screenshotIds / representativeScreenshotId は `frameCaptures[].id` から選んでください。実画像を直接読めない場合でも、timestampMsと文字起こしの流れから最も近いframe idを紐付けてください。
...
- 各User Storyには evidence を最低1つ入れてください。動画ID、秒単位のtRange、セグメントの短いtitle、segment summary、代表スクリーンショットID、関連スクリーンショットIDを入れてください。
- evidence.tRange は、User Storyに対応する操作が動画内で確認できる範囲を秒単位 [start, end] で指定してください。
...
- 最終JSONは次の意味で埋めてください。
  ...
  - storyCandidates: [{ id, epicId, title, role: { value, grounding }, goal, benefit, acceptanceCriteria, summary, userStory, asA, iWant, soThat, evidence: [{ videoId, title, summary, tRange, representativeScreenshotId, screenshotIds }], unverified, confidenceScore, confidence }]
