"""Tools for StoryVault Capability Structuring Agent."""
...
def _task_bucket(tool_context: Any) -> dict[str, Any]:
    ...
    return _as_dict(state.get("storyvault_capability_structuring"))
...
def build_capability_structure_package(
    *,
    ...
    return {
        "schemaVersion": "storyvault-capability-structure-v1",
...
def save_capability_structure(
    ...
    if writable is not None and hasattr(writable, "__setitem__"):
        bucket = _task_bucket(tool_context)
        writable["storyvault_capability_structuring"] = {
            **bucket,
            "phase": "review",
            "artifact": {
    ...
    return {
        ...
        "artifacts": [
            {
                "kind": "json_document",
                "title": "StoryVault Capability Structure Draft",
