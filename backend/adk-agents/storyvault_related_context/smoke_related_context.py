"""Smoke test StoryVault related-context GitHub PR flow.

Default is read-only:
  - load an existing StoryVault application + operation video from Firestore
  - build the exact `storyvault_related_context` mode_state
...
from storyvault_related_context.tools import (  # noqa: E402
...
def load_target(args: argparse.Namespace) -> SmokeTarget:
    ...
    apps = _collection(db, org_id, space_id, "storyVaultApplications")
    videos = _collection(db, org_id, space_id, "storyVaultOperationVideos")
    ...
    user_id = args.user_id or os.getenv("STORYVAULT_RELATED_CONTEXT_USER_ID", "")
    user_email = args.user_email or os.getenv("STORYVAULT_RELATED_CONTEXT_USER_EMAIL", "")
    if not user_id:
...
def invoke_adk(args: argparse.Namespace, target: SmokeTarget) -> dict[str, Any]:
    ...
    url = args.invoke_adk_url.rstrip("/") + "/v1/agents/storyvault_related_context/invoke"
    resp = requests.post(
        url,
        headers={
