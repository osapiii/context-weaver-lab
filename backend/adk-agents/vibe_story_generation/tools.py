"""Tools for VibeControl Story Generation Agent."""
...
def _task_bucket(tool_context: Any) -> dict[str, Any]:
    ...
    return _as_dict(state.get("vibe_story_generation"))
...
def build_story_generation_package(
    *,
    ...
    return {
        "schemaVersion": "vibe-control-story-generation-v1",
...
def save_story_generation(
    ...
    if writable is not None and hasattr(writable, "__setitem__"):
        bucket = _task_bucket(tool_context)
        writable["vibe_story_generation"] = {
            **bucket,
            "phase": "review",
            "artifact": {
    ...
    return {
        ...
        "artifacts": [
            {
                "kind": "json_document",
                "title": "VibeControl Story Generation Draft",
