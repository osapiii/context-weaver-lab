...
from storyvault_related_context.smoke_related_context import (
    SmokeTarget,
    build_mode_state,
    build_request_doc,
)
...
def test_build_mode_state_matches_related_context_bucket_shape():
    ...
    assert state["active_mode"] == "storyvault_related_context"
    bucket = state["storyvault_related_context"]
...
def test_build_request_doc_uses_adk_invoke_request_shape():
    ...
    assert doc["input"]["mode"] == "storyvault_related_context"
    assert doc["input"]["organizationId"] == "org-1"
    assert doc["input"]["spaceId"] == "space-1"
    assert doc["input"]["modeState"]["storyvault_related_context"]["setup"][
        "operation_video_id"
    ] == "video-1"
