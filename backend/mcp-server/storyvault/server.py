...
def _tool_definitions() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "description": "Fetch an operation-video-centered context bundle with signed video/screenshot URLs, source assets, linked user stories, evidence, GitHub pull requests, Slack messages, and knowledge documents. Use this when the user's request is ambiguous or video-oriented.",
            "inputSchema": {
                "type": "object",
        ...
        },
        {
            ...
            "description": "Generate story-centered HTML and Markdown context reports including media refs, GitHub pull requests, Slack messages, and knowledge documents. Use for explicit story work; otherwise prefer get_operation_video_context.",
            "inputSchema": {
                "type": "object",
...
def _store(principal: McpPrincipal) -> StoryVaultStore:
    if (
        principal.connection_id == "env-dev-token"
        and (
            os.getenv("STORYVAULT_MCP_DEV_FIXTURE", "").lower() in {"1", "true", "yes"}
            or os.getenv("STORYVAULT_MCP_DEV_FIXTURE", "").lower() in {"1", "true", "yes"}
        )
    ):
        return FixtureStoryVaultStore(principal)  # type: ignore[return-value]
    return StoryVaultStore(principal)


def _default_storage_bucket() -> str:
    return (
        str(os.getenv("STORYVAULT_MCP_DEFAULT_STORAGE_BUCKET") or "").strip()
        or str(os.getenv("STORYVAULT_MCP_DEFAULT_STORAGE_BUCKET") or "").strip()
...
def _report_bucket_name() -> str:
    return (
        ...
        or str(os.getenv("STORYVAULT_MCP_REPORT_BUCKET") or "").strip()
...
def _public_base_url() -> str:
    return (
        ...
        or "https://storyvault-mcp-mdgjayj74q-an.a.run.app"
    )


def _report_publisher(store: Any) -> Any:
    if isinstance(store, FixtureStoryVaultStore):
...
def _story_context_parts(store: StoryVaultStore, args: dict[str, Any]) -> dict[str, Any]:
...
def _get_story_context_raw(store: StoryVaultStore, args: dict[str, Any], *, format_name: str) -> str:
...
def _get_story_context(store: StoryVaultStore, args: dict[str, Any]) -> Any:
    bundle_args = _story_context_parts(store, args)
...
def _get_operation_video_context(store: StoryVaultStore, args: dict[str, Any]) -> Any:
    application_id = str(args.get("applicationId") or "")
...
def _read_resource(principal: McpPrincipal, uri: str) -> dict[str, Any]:
    store = _store(principal)
    if uri == "storyvault://applications":
        return {
            "contents": [
                {
                    "uri": uri,
    ...
    if uri.startswith("storyvault://applications/") and uri.endswith("/stories"):
        application_id = uri.removeprefix("storyvault://applications/").removesuffix("/stories")
        return {
            "contents": [
                {
                    "uri": uri,
    ...
    if uri.startswith("storyvault://stories/") and (uri.endswith("/context.md") or uri.endswith("/context.html")):
        is_markdown = uri.endswith("/context.md")
        story_id = uri.removeprefix("storyvault://stories/").removesuffix("/context.md").removesuffix("/context.html")
        story = store.get_story(story_id)
    ...
    if method == "resources/list":
        ...
        for application in _store(principal).list_applications(limit=50):
            ...
            if application_id:
                resources.append(
                    {
                        "uri": f"storyvault://applications/{application_id}/stories",
