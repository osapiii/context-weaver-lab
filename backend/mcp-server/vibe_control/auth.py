...
def _dev_principal_for_token(token: str) -> McpPrincipal | None:
    dev_token = (
        ...
        or os.getenv("VIBE_CONTROL_MCP_DEV_TOKEN", "").strip()
    )
    if not dev_token or token != dev_token:
        return None
    organization_id = os.getenv(
        "STORYVAULT_MCP_DEV_ORGANIZATION_ID",
        os.getenv("VIBE_CONTROL_MCP_DEV_ORGANIZATION_ID", "local-org"),
    )
    space_id = os.getenv(
        "STORYVAULT_MCP_DEV_SPACE_ID",
        os.getenv("VIBE_CONTROL_MCP_DEV_SPACE_ID", "local-space"),
    )
    scopes = _as_string_set(
        os.getenv(
            "STORYVAULT_MCP_DEV_SCOPES",
            os.getenv("VIBE_CONTROL_MCP_DEV_SCOPES", "context:read"),
        ).split(",")
    )
    allowed = _as_string_set(
        [
            ...
            for item in os.getenv(
                ...
                os.getenv("VIBE_CONTROL_MCP_DEV_APPLICATION_IDS", ""),
            ).split(",")
            if item.strip()
        ]
    )
    return McpPrincipal(
        ...
        external_agent=os.getenv(
            ...
            os.getenv("VIBE_CONTROL_MCP_DEV_EXTERNAL_AGENT", "codex-local"),
        ),
    )
...
def _connection_ref(db: firestore.Client, organization_id: str, space_id: str, connection_id: str):
    return (
        ...
        .collection("spaces")
        .document(space_id)
        .collection("vibeControlMcpConnections")
