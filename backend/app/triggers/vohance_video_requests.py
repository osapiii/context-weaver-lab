"""Vohance video RequestDoc triggers.

Thin Firebase Functions bridge:
Firestore RequestDoc -> corresponding Vohance Cloud Run worker.
The workers live under backend/microservice/vohance and keep the original
...
def _worker_url(env_name: str, route: str) -> str:
...
def _post_to_worker(
    *,
    ...
    if not request_id or not collection_path or not request_path:
        print("vohance video trigger: missing event data; skipping")
        return
    ...
    url = _worker_url(env_name, route)
    _patch(collection_path, request_id, {"status": "processing"})
    _append_log(collection_path, request_id, f"Vohance worker start: {route}", "info")
    try:
        ...
        _append_log(collection_path, request_id, f"Vohance worker completed: {route}", "info")
...
def on_auto_section_video_request_created(
    ...
    _post_to_worker(
        event=event,
        env_name="VOHANCE_AI_VIDEO_SECTIONING_URL",
        route="/auto-section",
        request_timeout=520,
    )
...
def on_split_video_request_created(
    ...
    _post_to_worker(
        event=event,
        env_name="VOHANCE_SPLIT_VIDEO_BY_TIMESTAMPS_URL",
        route="/split",
    )
...
def on_video_transcription_request_created(
    ...
    _post_to_worker(
        event=event,
        env_name="VOHANCE_TRANSCRIBE_AUDIO_WITH_GCP_STT_URL",
        route="/transcribe",
    )
...
def on_text_to_speech_request_created(
    ...
    _post_to_worker(
        event=event,
        env_name="VOHANCE_TEXT_TO_SPEECH_WITH_GOOGLE_URL",
        route="/synthesize",
    )
...
def on_merge_video_audio_narration_request_created(
    ...
    _post_to_worker(
        event=event,
        env_name="VOHANCE_MERGE_VIDEO_AUDIO_NARRATION_URL",
        route="/merge",
    )
...
def on_concatenate_section_videos_request_created(
    ...
    _post_to_worker(
        event=event,
        env_name="VOHANCE_CONCATENATE_SECTION_VIDEOS_URL",
        route="/concatenate",
    )
...
def on_add_video_subtitle_request_created(
    ...
    _post_to_worker(
        event=event,
        env_name="VOHANCE_ADD_VIDEO_SUBTITLE_URL",
        route="/add-subtitles",
    )
...
def on_trim_silence_video_request_created(
    ...
    _post_to_worker(
        event=event,
        env_name="VOHANCE_TRIM_SILENCE_VIDEO_URL",
        route="/trim-silence",
    )
