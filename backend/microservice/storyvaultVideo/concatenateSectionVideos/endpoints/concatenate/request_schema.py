...
class AudioSegmentInput(BaseModel):
    """セクション内に配置する音声ファイル"""
    sourceBucketName: str = Field(..., min_length=1, description="音声ファイルのGCSバケット名")
    sourceFilePath: str = Field(..., min_length=1, description="音声ファイルのGCSファイルパス")
    timestampMs: int = Field(..., ge=0, description="セクション先頭からの開始位置（ミリ秒）")
...
class SectionVideoPath(BaseModel):
    ...
    audioSegments: List[AudioSegmentInput] = Field(
        default_factory=list,
        description="このセクション内で合成するAI音声。指定時は動画タイムラインと音声タイムラインを別々に構築する"
    )
...
class ConcatenateRequestInput(BaseModel):
    ...
    sectionVideoPaths: List[SectionVideoPath] = Field(
        ...
        min_length=1,
        examples=[[
            {"bucketName": "storyvault-videos", "filePath": "sections/section-0/merged_video.mp4"},
            {"bucketName": "storyvault-videos", "filePath": "sections/section-1/merged_video.mp4"}
        ]]
    )
