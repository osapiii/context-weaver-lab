...
class RequestContext:
    ...
    def get_segment_output_path(self, segment_number: int) -> str:
        """セグメントの出力パスを取得（outputFilePath配列から直接取得）"""
        input_data = self.get_param('input', {})
        output_file_paths = input_data.get('outputFilePath', [])
        
        # セグメント番号（1始まり）から配列インデックス（0始まり）を計算
        array_index = segment_number - 1
        
        logger.info(f"🔍 get_segment_output_path デバッグ:")
        logger.info(f"  - segment_number: {segment_number}")
        logger.info(f"  - array_index: {array_index}")
        logger.info(f"  - output_file_paths length: {len(output_file_paths)}")
        logger.info(f"  - output_file_paths: {output_file_paths}")
        
        if array_index < 0 or array_index >= len(output_file_paths):
            error_msg = (
                f"segment_number ({segment_number}) is out of range for outputFilePath array "
                f"(length: {len(output_file_paths)}). "
                f"Expected range: 1 to {len(output_file_paths)}"
            )
            logger.error(error_msg)
            raise ValueError(error_msg)
        
        output_path = output_file_paths[array_index]
        logger.info(f"  - ✅ 出力パス: {output_path}")
        return output_path

    # splitVideoByTimestamps固有のプロパティ（後方互換性のため保持）
    @property
    ...
    @property
    def output_gcs_file_path(self) -> str:
        """出力ファイルパスを取得（input/metadata構造対応）"""
        input_data = self.get_param('input', {})
        return input_data.get('outputFilePath', '')

    @property
    def cutoff_seconds(self) -> list:
        """cutoffSecondsを取得（input/metadata構造対応）"""
        input_data = self.get_param('input', {})
        return input_data.get('cutoffSeconds', [])
...
class Context:
    ...
    def __init__(
        self,
        service_name: str = "concatenate-section-videos",
    ...
    def validate_config(self) -> tuple[bool, List[str]]:
        ...
        if self.max_segments <= 0:
            errors.append(f"max_segments must be positive: {self.max_segments}")
    ...
    def create_request_context(
        ...
        # セグメント数の検証（splitVideoByTimestamps固有）
        cutoff_seconds = ctx.get_param('cutoffSeconds', [])
        if cutoff_seconds and len(cutoff_seconds) + 1 > self.max_segments:
            raise ValueError(f"Too many segments: {len(cutoff_seconds) + 1} > {self.max_segments}")
