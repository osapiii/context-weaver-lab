...
このサービスは、指定されたタイムスタンプで動画を分割し、
Google Cloud Storage に保存する Web API を提供します。

主な機能:
- GCS から動画をダウンロード
- 指定されたタイムスタンプで動画を分割
- 分割された動画を GCS にアップロード
- Firestore への進捗ログ記録
"""
...
try:
    ...
    from endpoints.split import execute as split_execute
...
def split():
    """動画分割エンドポイント - endpoints/split/execute.pyに委譲"""
    return split_execute.handle(g.request_context)
...
if __name__ == "__main__":
    try:
        ...
        logger.info(f"✂️  動画分割: http://localhost:{port}/split")
