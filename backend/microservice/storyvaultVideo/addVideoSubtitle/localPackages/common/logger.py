...
class StructuredLogger:
    def __init__(self, service_name: str = "add-video-subtitle"):
    ...
    def api_response(self, status_code: int, details: str = "", duration_ms: Optional[float] = None):
        ...
        if 200 <= status_code < 300:
            self.success("📨 APIレスポンス受信", **log_data)
        else:
            self.error("📨 APIエラーレスポンス", **log_data)
...
logger = StructuredLogger("add-video-subtitle")
