...
def test_connection() -> bool:
    ...
    try:
        client = get_client()
        # コレクション一覧を取得してみる（最初の1つだけ）
        for _ in client.collections(page_size=1):
            break
        return True
    except Exception:
        return False
...
# ============================================
# 進捗ログ記録機能（splitVideoByTimestamps固有）
# ============================================
...
def log_processing_status(
    ...
    try:
        ...
        # ドキュメントリファレンス取得
        # ctx.collection_nameは完全なコレクションパス（例: organizations/{orgId}/spaces/{spaceId}/requests/splitVideoRequests/logs）
        ...
        # ログタイプの決定（errorがある場合はerror、それ以外はinfo）
...
def log_segment_progress(
    ...
    try:
        ...
        logger.error(f"セグメント進捗記録エラー", error=e, segment=segment_number)
        return False
...
def get_processing_status(
    ...
    try:
        ...
        logger.error("ステータス取得エラー", error=e)
        return None
...
def batch_update_logs(
    ...
    params: {
        updates: Dict[str, Dict[str, Any]] - {document_path: update_data}の辞書
    }

    returns: bool - 更新成功の場合True
    """
    try:
        client = get_client()
        batch = client.batch()

        for doc_path, update_data in updates.items():
            # パスを分解してドキュメントリファレンスを取得
            path_parts = doc_path.split('/')
            if len(path_parts) % 2 != 0:
                logger.warning(f"無効なドキュメントパス: {doc_path}")
                continue
