...
class RequestContext:
    ...
    def set_request(self, request) -> None:
        ...
        if request.is_json:
            # force=Trueで確実にJSONを取得（複数回呼び出し可能）
...
class Context:
    ...
    def create_request_context(
        ...
        raises: ValueError - 必須パラメータが不足している場合
    ...
    def debug_dump(self, request_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        params: {
            request_id: Optional[str] - 特定のリクエストIDのみダンプ
        }

        returns: Dict[str, Any] - コンテキスト情報
        """
        dump = {
...
# サービス名とバージョンは各マイクロサービスで設定してください
context = Context(
    service_name="text-to-speech-with-google",
    service_version="2.0.0",
    port=int(os.getenv("PORT", "8080")),
    debug_mode=os.getenv("DEBUG", "false").lower() == "true"
)

# Health endpoint用の追加属性（後方互換性）
context.default_voice_name = "Zephyr"
context.default_language_code = "ja-JP"
context.default_audio_encoding = "MP3"
context.max_text_length = 5000
...
def get_auth_method_for_context() -> str:
    """
    認証方法を取得（後方互換性用）
...
# Context インスタンスにメソッドを追加
context.get_auth_method = get_auth_method_for_context
