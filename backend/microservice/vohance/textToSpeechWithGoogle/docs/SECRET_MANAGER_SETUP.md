...
```bash
gcloud services enable secretmanager.googleapis.com --project=vohance-dev
```
...
# IAM権限付与
PROJECT_NUMBER=$(gcloud projects describe vohance-dev --format="value(projectNumber)")
...
```bash
gcloud secrets list --project=vohance-dev --filter="name~gemini-api-key"
```

### 5. Secretの内容を確認（デバッグ用）

```bash
gcloud secrets versions access latest --secret="gemini-api-key-org_default" --project=vohance-dev
```
...
echo "GOOGLE_CLOUD_PROJECT=vohance-dev" >> .env
...
```bash
gcloud secrets delete gemini-api-key-org_default --project=vohance-dev
```
