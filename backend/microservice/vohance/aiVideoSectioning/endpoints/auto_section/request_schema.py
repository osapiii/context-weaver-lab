"""
/auto-section エンドポイントのリクエストスキーマ
...
class AutoSectionRequestInput(BaseModel):
    """
    ...
    sectioningPrompt: Optional[str] = Field(
        default=None,
        description="ユーザー指定のセクション分割指示（任意）"
    )


class AutoSectionRequestSystemMetadata(BaseModel):
    """
    ...
    isLlmCall: bool = Field(
        ...,
        description="LLM呼び出しフラグ（Gemini使用のためtrue）"
    )
...
class AutoSectionRequest(BaseModel):
    """
    /auto-section エンドポイントのリクエストスキーマ（RequestDoc黄金テンプレート準拠）
    ...
    input: AutoSectionRequestInput = Field(
        ...,
        description="入力パラメータ（Command）"
    )

    systemMetadata: AutoSectionRequestSystemMetadata = Field(
        ...,
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                "request_id": "autoSectionVideoRequest_1234567890",
                "input": {
                ...
                "systemMetadata": {
                    ...
                    "loggingDocumentId": "autoSectionVideoLogs",
