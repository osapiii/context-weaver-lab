...
class ParentPathManager:
    ...
    def build_firestore_collection_path(
        ...
        params: {
            ...
            subcollection: str | None - サブコレクション名（オプション）
        }
    ...
    def build_gcs_parent_path(
        ...
        params: {
            ...
            additional_segments: list[str] | None - 追加セグメント（オプション）
        }
    ...
    def validate_system_metadata(self, params: dict) -> tuple[bool, str | None]:
        """
        ...
        returns: tuple[bool, str | None] - (検証成功, エラーメッセージ)
        ...
        # 初期化時の ID と一致するかチェック
        if system_metadata["organizationId"] != self.organization_id:
            return (
                False,
                f"systemMetadata.organizationId mismatch: expected {self.organization_id}, got {system_metadata['organizationId']}",
            )
        if system_metadata["spaceId"] != self.space_id:
            return (
                False,
                f"systemMetadata.spaceId mismatch: expected {self.space_id}, got {system_metadata['spaceId']}",
            )

        # Logging 用フィールド（オプション）
...
# グローバルインスタンスは作成しない（organizationId/spaceIdが必要なため）
# 各エンドポイントで ParentPathManager({"organization_id": ..., "space_id": ...}) を初期化して使用
