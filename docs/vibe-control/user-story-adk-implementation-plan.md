# VibeControl User Story ADK Implementation Plan

## 1. Basic Stance

VibeControl のユーザーストーリー生成は、単一の長時間 Job ではなく、対話型 Workbench と分離 ADK によって進める。
...
- ユーザー承認前の変更を `vibeControlDraftPatches` に保存する。
- ユーザー承認後に `vibeControlCapabilities` へ反映する。

Main tools:

- `read_vibe_control_session`
- `list_evidence_cards`
...
- draft patch を保存し、承認後に `vibeControlStories` と `vibeControlStoryEvidence` へ反映する。
...
Existing collections can be extended. New collections should stay under the same organization/space base path used by VibeControl.

```text
organizations/{orgId}/spaces/{spaceId}/
  vibeControlApplications/{applicationId}
  vibeControlCapabilities/{capabilityId}
  vibeControlStories/{storyId}
  vibeControlStoryEvidence/{evidenceId}
  vibeControlSourceConnections/{sourceConnectionId}
  vibeControlOperationVideos/{videoId}
  vibeControlGenerationSessions/{generationSessionId}
  vibeControlDraftPatches/{patchId}
  vibeControlSourceAssets/{sourceAssetId}
```

### 3.1 Capability

```ts
type VibeControlCapability = {
  id: string;
...
Add these fields to `VibeControlStory`.

```ts
type VibeControlStoryCapabilityFields = {
  capabilityId: string;
  capabilityKey: string;
...
Extend current `VibeControlStoryEvidence` so it can support Capability first.

```ts
type VibeControlEvidence = {
  id: string;
  applicationId: string;
  applicationKey: string;
...
```ts
type VibeControlGenerationSession = {
  id: string;
  applicationId: string;
  applicationKey: string;
...
```bash
ADK_APP_NAMES=vibe_control_capability,vibe_control_story,...
VIBE_CONTROL_CAPABILITY_AGENT_URL=https://...
VIBE_CONTROL_STORY_AGENT_URL=https://...
```
...
2. Create `VibeControlSourceAsset` with `sourceType = application_screenshot`.
...
Existing VibeControl page can gain a new workbench view:

```text
/admin/vibe-control?view=story-workbench&applicationId=...
```

Main component:

```text
VibeControlStoryWorkbench.vue
```

Child components:

```text
VibeControlWorkbenchChat.vue
VibeControlEvidenceInbox.vue
VibeControlCapabilityTree.vue
VibeControlStoryDraftBoard.vue
VibeControlPatchReviewPanel.vue
VibeControlMediaIngestStatus.vue
```
...
```text
+--------------------------------------------------------------------------------+
| VibeControl / Application: Payroll Admin                         [Sync Sources] |
+--------------------------------------------------------------------------------+
...
- Add `VibeControlCapability` schema and converter.
...
- Add `backend/adk-agents/vibe_control_capability`.
...
- Applying patch updates `vibeControlCapabilities`.

### Phase 3: Story Agent Service

Deliverables:

- Add `backend/adk-agents/vibe_control_story`.
...
1. Add `vibe-control-capability-agent`.
...
1. Add `vibe-control-story-agent`.
