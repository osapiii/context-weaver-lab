...
import type { StoryVaultFilters } from "@stores/storyVault";
import type {
  DecodedStoryVaultApplication,
  DecodedStoryVaultStory,
} from "@models/storyVault";
import {
  STORYVAULT_DRIFT_LABELS,
  STORYVAULT_STATUS_LABELS,
} from "@models/storyVault";
...
const emit = defineEmits<{
  "update-filter": [
    key: keyof StoryVaultFilters,
    value: StoryVaultFilters[keyof StoryVaultFilters],
  ];
  "clear-filters": [];
}>();
...
const statusOptions = computed(() => [
  { label: "すべて", value: "all" },
  ...Object.entries(STORYVAULT_STATUS_LABELS).map(([value, label]) => ({
    label,
    value,
  })),
]);
...
const driftOptions = computed(() => [
  { label: "すべて", value: "all" },
  ...Object.entries(STORYVAULT_DRIFT_LABELS).map(([value, label]) => ({
    label,
    value,
  })),
]);
...
const activeFilterChips = computed(() => {
  ...
  if (props.filters.status !== "all") {
    chips.push(STORYVAULT_STATUS_LABELS[props.filters.status]);
  }
  ...
  if (props.filters.drift !== "all") {
    chips.push(`drift:${STORYVAULT_DRIFT_LABELS[props.filters.drift]}`);
  }
...
function emitFilter<K extends keyof StoryVaultFilters>(
  key: K,
